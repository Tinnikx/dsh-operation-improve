...
var ROOT_CLASS2 = "dsh-oi-nav";
var VIEW_AREA_SELECTOR = '[class*="_viewArea"]';
var RIGHT_RATIO = 0.02;
var HOT_ZONE = 72;
var MIN_ANCHORS = 2;
var SCROLL_MARGIN = 16;
var SUMMARY_MAX = 90;
function installStartNav(options) {
  const hotZone = options?.hotZone ?? HOT_ZONE;
  const minAnchors = options?.minAnchors ?? MIN_ANCHORS;
  const root = document.createElement("div");
  root.className = ROOT_CLASS2;
  root.setAttribute("aria-label", "\u5BF9\u8BDD\u8D77\u70B9\u5BFC\u822A");
  const tooltip = document.createElement("div");
  tooltip.className = `${ROOT_CLASS2}__tip`;
  root.append(tooltip);
  document.body.append(root);
  let anchors = [];
  let scroller = null;
  ...
  const measureCovered = () => {
    ...
    for (const el of document.elementsFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2)) {
      if (el.closest(`.${ROOT_CLASS2}`) !== null) continue;
  ...
  function rebuild() {
    ...
    const unchanged = found.length === anchors.length && found.every((entry, i) => entry.element === anchors[i].element && entry.summary === anchors[i].summary);
    if (unchanged) {
      refreshCovered();
      syncActive();
      return;
    ...
    anchors = found.map((entry, index) => {
      ...
      tick.type = "button";
      tick.className = `${ROOT_CLASS2}__tick`;
  ...
  function scrollToAnchor(index) {
    ...
    if (item === void 0 || scroller === null) return;
  ...
  const observer = new MutationObserver((records) => {
    for (const record of records) {
      ...
      if (target instanceof Element && target.closest(`.${ROOT_CLASS2}`) !== null) continue;
      queueRebuild();
      return;
    }
  });
...
function collectAnchors(scroller) {
  const out = [];
...
.${ROOT_CLASS2}[data-visible] { opacity: 1; }
.${ROOT_CLASS2}[data-empty] { display: none; }
.${ROOT_CLASS2}__tick {
  width: 30px;
  height: 6px;
  padding: 0;
...
.${ROOT_CLASS2}[data-visible] .${ROOT_CLASS2}__tick { pointer-events: auto; }
.${ROOT_CLASS2}__tick:hover { width: 44px; opacity: 0.85; }
.${ROOT_CLASS2}__tick[data-active] {
  width: 44px;
  opacity: 1;
...
.${ROOT_CLASS2}__tip[data-open] { opacity: 1; }
`;
