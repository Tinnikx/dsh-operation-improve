...
- **web 产品里整条 entry 被关掉的两张**：`time-context`（`refreshIntervalMs`）与 `schedule`（`deliveryHistoryDays` / `deliveryHistoryRecords`）。`dsh-web-app/cordis.patch.yml` 给这两行都写了 `disabled: true`，而 rc.2 的全部 `cordis.patch.yml` 里没有任何一层把它们重新启用——config 有活的消费者（两枚键在各自的 `dsh-*/lib/index.js` 里都被现读），但那条 entry 在 web 组合里根本不实例化，面板写下去到不了任何会话（口径 #2）。
- **config 块里挂着 `!!js` 表达式的一张**：`deepseek-account` 的 `balanceTimeoutMs`。`dsh-base/cordis.patch.yml` 给这条 entry 写了 `desktopPlatform: !!js "ctx.get('profileContext')?.name === 'desktop' && …"`，而面板重述同一条 config 时把**求值结果**当普通标量写回托管区段——等于把「每次启动按环境判定」冻成「按点保存那一次判定」，静默改掉行为（口径 #4）。这条键自己是有消费者的（`dsh-deepseek-account-platform/lib/index.js` 里 `this.balanceTimeout = resolved.balanceTimeoutMs`）。
...
- **client 侧条目的 config**（`ui-plugin-manager` 的 `registryProbeEnabled` / `registryProbeTimeoutMs` / `registryProbeCacheTtlMs`，以及 rc.2 新增的 `shortcuts.stopSequenceMs` 与 `ui-settings-account` 的 `bonusAckRetryDelayMs` / `bonusAckRetryMaxDelayMs` / `developerTools`）：这些键只在浏览器半边读，而 `window.__DSH_BOOT__` 的条目只带 `id` / `url` / `rev` / `inject`——profile 用户 patch 层的 config 会不会流进 client 树没有查实，口径 #2 要「真有活消费者」，未证实就不收（写下去可能是空操作）。不是双入口问题：插件管理页也没渲染这三枚，`plugins.bundle.config` 那个槽要条目自己注册表单，`dsh-client-ui-plugin-manager` 没注册。
- **字段里有 `__jsExpr` 表达式的**（`tools`、`sandbox-policy`、`approval`、`webserver` 等）：重述时会被序列化成普通映射，静默改行为（口径 #4）。
- **值不是数字或布尔的**：字符串、枚举、数组表格面板表达不了，写进托管区段还要原样重述，猜出来的 YAML 比不写更糟（口径 #3）。rc.2 新增的这批里被这条拦下的是 `llm-deepseek-account` 的 `baseURL` / `thinking` / `reasoningEffort` / `models` / `retryPolicy`、`time-context.timeZone`、`ui-settings-account` 的 `version` / `step` / `purpose` / `process` / `completion` / `usage`。
- **在 web profile 的组合里根本不存在的**：`tool-str-replace-editor`。`applyOps` 的 present 检查会拒，留着只是死重。

## 实测读数

一轮完整跑（`passed=26 failed=0 skipped=0 total=26`）：面板渲染出 75 个 `data-field`（清单 77 个字段，`system-prompt` 那两个布尔走复选框，`fields.jsx` 只给文本框挂 `data-field`）、16 张卡，`[data-save]` 计数为 `0`，每张卡都带着 `data-effect` 生效标记（「会话检索」卡整卡与它 6 个字段级标记全是「重启生效」——热重挂缺陷的处置，见[已知限制](#已知限制)；`bash-sandbox.timeoutMs` 是 `immediate`）；没人设过的 `persistedReadConcurrency` 那一行 `data-default` 在、`value` 为空、`placeholder` 是 `4`、输入框计算 `opacity` `0.55` 而整行与标签都是 `1`，手写的 `defaultLimit: 30` 那一行输入框 `opacity` `1`；`bash-sandbox.timeoutMs` 的徽标 `data-source="bundle"`、文字 `dsh-base`、`title` `@deepseek-ai/dsh-base`，与 host 归因出的包名一致，而手写行仍是 `手写`、系统默认行仍是 `系统默认`（两者都没有 `data-owner`）。只让输入框失焦、不点任何按钮，区段头写成 `# managed: {"session-query-sqlite":["maxLimit","readWindowMax"]}`，区段外逐字节不变；热重载把从基线算出的 `maxLimit: 50` / `readWindowMax: 50` 送进 loader 用了 **约 2.1s**（watcher 防抖之后），那一行随即变成 `opacity` `1` 且徽标转 `本面板`；写 `snippetChars` 时 bundle 层的 `path: ":memory:"` 与 `openAt: never` 原样重述进区段，三个键都进了 live config（**约 2.1s**）；手写那两行（`defaultLimit: 30      # 检索默认每页 30 条,…` 与 `readWindowMax: 40     # …`）连行尾中文注释原样保留；`defaultLimit = 60 > 托管中的 maxLimit = 50` 被前端拦下（`会话检索：默认每页条数不能超过每页条数上限，否则 session-query-sqlite 加载失败。`），文件 sha 前后同为 `588cabd61866515d`，点「清除」撤掉这条草稿后错误清空、待提交归零、输入框退回 `30`、文件 sha 不变；`imageOffloadByteQuantum` 写成 `maxRequestFilesBytes + 1` 同样被拦（`DeepSeek 模型接入：图片转存字节步长（Files 引用）不能超过单次请求文件总上限，否则 llm-deepseek 加载失败。`）、撤销后错误清零且文件没动；`fileRefreshMarginSeconds` 写成**等于**保留时长也被拦（`…必须严格小于上传文件保留时长…`，等号不在合法那一侧）；卸载副本里区段仍在、`maxLimit` 仍是 `50`，3182 上的 harness 起得来且名册里没有本插件；点「清除」之后不做任何别的操作，`snippetChars` **约 2s** 内从 live config 里消失、区段里也不含这个键（**不是把默认值写进去**）；全部清空后文件逐字节回到基线（sha `0ab14465ed271f64`），`maxLimit` 回落到手写层的 `100`、`path` 仍是 `:memory:`，`snippetChars` 那一行的输入框重新淡化成 `opacity` `0.55`；第 14 卡那一对：写 `plugin-manager.idleTimeoutMs` = `45000` 时区段头写成 `# managed: {"plugin-manager":["idleTimeoutMs"]}`，**约 2.1s** 后 loader 读到 `45000`，点「清除」后该键从 loader 里消失（host 路由的 `live` 是原始合成 config，不含上游 zod 的默认值，与 `snippetChars` 同一口径）且文件逐字节回到基线。rc.2 那两张新卡（12a–12d）：写 `llm-deepseek-account.maxTokens` = `128000` 后**只有账号那条 entry** 读到 `128000`，`llm-deepseek`（API key 那一路）同一次读数里是 `null`，区段头 `# managed: {"llm-deepseek-account":["maxTokens"]}`，**约 2.1s**；清除后 loader 里 `maxTokens` 消失、文件逐字节回到基线。写 `session-log-deepseek.maxBytes` = `262144` 时区段头 `# managed: {"session-log-deepseek":["maxBytes"]}`、**约 2.1s** 后 loader 读到 `262144`；清除后同样摘干净并回到基线。

## 与上游 schema 对齐（升级必检）

清单声明的**信息格式**（每个字段的 `type`、`default`、`min`/`max`）必须与所锚定 harness 的 config schema 一致。这条由 [scripts/check-catalog-schema.mjs](../scripts/check-catalog-schema.mjs) 把关：`npm run check:catalog` 用测试栈那份 home 跑权威 harness 的 `--dump-config-schema`，把 [CATALOG](../src/harness-config/catalog-entries.js) 逐键对到上游 `$defs/configN` 的对应属性上，漂移即非零退出。

比对契约（口径：**镜像 + 保留防呆**）：

- **`type` 必须相等**。上游 `z.number()` 的键，catalog 不许标 `integer`——标了就会拒掉上游接受的值（`bash-sandbox.timeoutMs` 是 `120000` 而不是非得整数）。`integer-list` 只对到上游的数组容器，数组内元素是不是整数属面板口径，不比。
- **`default` 上游有则必须相等**；上游 schema 无 `default` 的键（如 `spill-policy.maxInlineTokens`，其 `12500` 来自 bundle 层）跳过——清单的 `default` 只是界面提示，从不写盘。
- **上游有的边界 catalog 必须有、且不更松**：上游 `minimum` ⇒ catalog `min` 存在且 `≥`；上游 `maximum` ⇒ catalog `max` 存在且 `≤`。堵的是最危险的方向——面板放行一个上游会拒、写下去整棵树起不来的值（`llm-deepseek.maxTokens` 上游 `maximum: 2^53-1`，漏标就会让一个 `2^53` 的整数过闸）。
- **catalog 比上游更严的防呆**（上游无界而清单加了 `min: 1`、`graceMs` 的 `max: 2^31-1`）**不比对**，允许——那些是面板自己挡退化值的，不是上游硬抛。

这条**不覆盖**跨字段约束（`crossRules`）与「这条 entry 在 web 产品里还活着吗」：上游的 `if (a > b) throw` 与 `disabled: true` 名单不在 schema dump 的 `properties` 里，仍是手抄，靠下面的落点重读。

**每次升级 harness 必跑**：换锚定版本的那一轮，除了既有的行为级验证，还要 `npm run check:catalog`。新版本改了某个键的类型、值域或默认，这条会当场红，逼你回去对上游重抄——它就是把「手抄不可验证」变成「手抄错了有门禁」的那道闸。

## 已知限制

- [精选清单](../src/harness-config/catalog-entries.js)是手抄的。每个字段的 `type` / `default` / `min` / `max` 与上游 rc.1 的 schema 是否一致，由 [npm run check:catalog](../scripts/check-catalog-schema.mjs) 逐键比对（见[与上游 schema 对齐](#与上游-schema-对齐升级必检)）；改锚定版本那一轮必跑，漂移即非零退出。**它不覆盖**跨字段约束（`crossRules`）与「这条 entry 在 web 产品里还活着吗」——上游的 `if (a > b) throw` 与 `disabled: true` 名单不在 schema dump 的 `properties` 里，仍要手抄。这三个落点是重读上游**发行包**（不是 dev 仓）的唯一凭据：键名与值域看各包 `lib/*.js` 里的 `Config = z.object({…})`（沙箱那两张卡的 schema 在 `dsh-bash-local` / `dsh-pwsh-local` 的 `static Config` 上，`dsh-*-sandbox` 只是继承它的执行器），`default` 看 `@deepseek-ai/dsh-base/cordis.patch.yml` 里该 entry 的 bundle 层值，活没活看 `dsh-web-app/cordis.patch.yml` 的 `disabled: true` 名单。`verify:settings` 看不出这类漂移——它比的是面板显示的值与 host 现算的值，两边同源；[tests/catalog.test.mjs](../tests/catalog.test.mjs) 兜的是「抄下来之后自洽」（规则引用的键在本卡、全默认值不冲突、每个默认值过自己的边界），也不是「抄得对」。真正比对上游的是 check:catalog。
