...
- [精选清单](../src/harness-config/catalog-entries.js)是手抄的，没有编译期保障。上游给某个 entry 改键名、改值域、改跨字段约束，面板都不会知道：改键名的表现是那一行写下去不起作用（patch 里多一个没人读的键），改约束的表现更糟——面板放行了一个会让插件加载失败的值，而 patch 是热的，保存那一刻整棵树就起不来。跨字段约束一律写成 `crossRules` 跑在合成值上，**不拿 `min` 去镜像另一个键的默认值**——那种镜像在上游改掉那个默认后就连方向都不对了（`llm-deepseek` 那三条曾经是这样，0.1.7 把三个步长与余量收进清单之后改成了四条 `crossRules`）。跟上的办法只有重读上游那几个包的 schema；对**发行包**（不是 dev 仓）重读的三个落点：键名与值域看各包 `lib/*.js` 里的 `Config = z.object({…})`（沙箱那两张卡的 schema 在 `dsh-bash-local` / `dsh-pwsh-local` 的 `static Config` 上，`dsh-*-sandbox` 只是继承它的执行器），`default` 看 `@deepseek-ai/dsh-base/cordis.patch.yml` 里该 entry 的 bundle 层值，「这条 entry 在 web 产品里还活着吗」看 `dsh-web-app/cordis.patch.yml` 的 `disabled: true` 名单。抄来的值与真值不等时，`verify:settings` 看不出问题——它比的是面板显示的值与 host 现算的值，两边同源；能兜住的是 [tests/catalog.test.mjs](../tests/catalog.test.mjs)：规则里引用的键必须在本卡、全默认值不自相冲突、每个默认值过自己的边界，这三条对的是「抄下来之后自洽」，不是「抄得对」。
