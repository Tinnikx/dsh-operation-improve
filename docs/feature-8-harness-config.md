...
- **client 侧条目的 config**（`ui-plugin-manager` 的 `registryProbeEnabled` / `registryProbeTimeoutMs` / `registryProbeCacheTtlMs`）：这三枚只在浏览器半边读，而 `window.__DSH_BOOT__` 的条目只带 `id` / `url` / `rev` / `inject`——profile 用户 patch 层的 config 会不会流进 client 树没有查实，口径 #2 要「真有活消费者」，未证实就不收（写下去可能是空操作）。不是双入口问题：插件管理页也没渲染这三枚，`plugins.bundle.config` 那个槽要条目自己注册表单，`dsh-client-ui-plugin-manager` 没注册。
