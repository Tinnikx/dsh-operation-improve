...
**置顶项跟着行的置顶态翻转**（未置顶 → 「置顶会话」+ 空心图钉 → `workspaces.pinSession`；已置顶 → 「取消置顶」+ 实心图钉 → `workspaces.unpinSession`）。置顶态在开菜单那一刻读 `workspaces.list.getSnapshot().pinnedSessionIds`，不订阅不缓存——和文案同一时刻求值，才跟得上别处的改动。归档行上游不渲染这一项（菜单与 hover 按钮都不），这里同样不给，判据取自同一快照的 `archivedSessionIds`。置顶不是破坏性操作，不给它配批量版。

**右键菜单不提供归档**（单选没有「归档会话」，多选也没有批量归档），归档一律走行自己的「...」。理由不是风格：上游那一项不是「调一次服务」——会话有进行中的工作时第一次 `archiveSession` 会被 host 拒（`WorkspaceArchiveError`，`rpcError.code === 'workspace/session-active'`），上游捕获后弹「停止并归档此会话？」列出将被停的回合 / 子代理 / 后台任务 / 定时提醒，确认了才带 `{ stopActivity: true }` 重试。插件只发第一次调用、`run()` 又不 catch，结果是对这类会话**静默没反应**（控制台留一次未处理 rejection）。要么整条链抄下来（含那个对话框与 activity 列表），要么让位——这里选择让位。**取消归档留着**：它不带 options、不会被拒、不是破坏性操作，这一份复制是完整的。

连带后果写清楚：会话多选失去了唯一的批量动作（`sessions` 契约上也没有 delete），选中、计数与高亮照常，右键那一层整个交回页面。

**归档行的末项是「取消归档」**（`menu.unarchiveSession` + `IconUnarchiveOutlineRegular` → `workspaces.unarchiveSession`）：上游这一项与归档项共用同一个 slot（order 400），按归档态换文案、图标与方法。该 home 里没有归档会话时这一项无从对照，`verify` 的归档行那两条会 FAIL 并点名先去归档一条会话（见[验证](./verify.md#端到端)）。

**这两处翻转都是手抄，不是从上游菜单读的**：上游在侧边栏行上没有任何右键菜单（`dsh-client-ui-workspace` 里搜 `contextmenu` / `onContextMenu` 是 0 命中，0.1.6 源码同样为 0），被对齐的那份「...」是点按钮开的另一棵 DOM；插件弹的是按 `buildItems` 现搭的一份，所以「上游在某状态下给哪几项」这件事没有任何运行时通道——上游跟着状态变的，这边一律要跟着改代码。状态位本身（`pinnedSessionIds` / `archivedSessionIds`）读的是上游快照，项的**集合**不读。漂移的表现因此不是报错而是「和那个『...』菜单不一样」，哨兵只有 `verify` 里那几条逐项比对。
...
- 归档只能从行自己的「...」或 hover 按钮进。右键菜单故意不提供这一项（上游那条链的失败分支没抄，见[菜单项与服务映射](#菜单项与服务映射)），所以「右键 = 一步归档」这个捷径在这里不存在。**"让位"没有对应的上游右键菜单**：0.1.7 的 `dsh-client-ui-workspace` 里没有任何 `contextmenu` 监听，桌面壳也只给托盘设了原生菜单，插件不接之后落到的是浏览器默认行为。
