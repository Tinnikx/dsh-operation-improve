 ...
 * 手写的 `session-query-sqlite`）。断言全部对**字节**，不是对「解析出来一样」：区段外
 * 任何一个字节的改动都会丢掉别人写在行尾的注释，而 YAML 解析器看不出这个区别。
 */
...
const QUERY = { id: 'session-query-sqlite', managed: [['defaultLimit', 25]], restated: [['readWindowMax', 40], ['openAt', 'never']] }
const SANDBOX = { id: 'bash-sandbox', managed: [['timeoutMs', 45000]], restated: [] }

test('加托管区段：区段外逐字节不变', () => {
  const after = applyToText(FIXTURE, [QUERY, SANDBOX])
  ...
  // 手写那三行连同行尾中文注释原样还在。
  assert.ok(after.includes('    defaultLimit: 30      # 检索默认每页 30 条,侧栏翻页少一页'))
  assert.ok(after.includes('    readWindowMax: 40     # 单次读取窗口跟着默认条数走,略留余量'))
  // 重述的键必须在区段里出现，否则 whole-config 替换会把它们抹掉。
  assert.ok(parts.section.includes('openAt: "never"'), '重述键要带进区段')
  assert.equal(countSectionItems(parts.section), 2)
  assert.deepEqual(readManagedHeader(parts.section), {
    'session-query-sqlite': ['defaultLimit'],
    'bash-sandbox': ['timeoutMs'],
  })
})

test('改一个字段：只有区段内变化', () => {
  const first = applyToText(FIXTURE, [QUERY, SANDBOX])
  const second = applyToText(first, [
    QUERY,
    { ...SANDBOX, managed: [['timeoutMs', 48000]] },
  ])
  ...
  assert.equal(
    a.section.replace('timeoutMs: 45000', 'timeoutMs: 48000'),
    b.section,
    '区段内也只有那一个数变了',
  )
})
...
test('清掉最后一个字段：区段整体消失，文件回到原始字节', () => {
  const withSection = applyToText(FIXTURE, [QUERY, SANDBOX])
  const onlySandbox = applyToText(withSection, [SANDBOX])
  assert.equal(countSectionItems(splitManaged(onlySandbox).section), 1)
  const cleared = applyToText(onlySandbox, [])
...
test('区段清空后若文档再没有顶层条目，补一行裸 []', () => {
  ...
  const withSection = applyToText(template, [SANDBOX])
...
test('落盘是同目录 rename：目标 inode 换新，不存在半份内容', () => {
  ...
  const next = applyToText(FIXTURE, [QUERY, SANDBOX])
